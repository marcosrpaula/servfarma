...
export class ListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._portfolioChart(
      '["--vz-info", "--vz-primary", "--vz-danger", "--vz-danger", "--vz-info"]',
    );
  }
  ...
  createJob() {
    if (this.jobData.valid) {
      if (this.jobData.get('id')?.value) {
        this.joblists = this.joblists.map((data: { id: any }) =>
          data.id === this.jobData.get('id')?.value ? { ...data, ...this.jobData.value } : data,
        );
