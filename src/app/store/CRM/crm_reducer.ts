...
import {
  ...
  deleteLeadSuccess,
  fetchCrmCompanyData,
  fetchCrmCompanyFailure,
  fetchCrmCompanySuccess,
  fetchCrmContactData,
  fetchCrmContactFailure,
  fetchCrmContactSuccess,
  fetchCrmDealData,
  fetchCrmDealFailure,
  fetchCrmDealSuccess,
  fetchCrmLeadData,
...
export const CRMReducer = createReducer(
  ...
  on(updateContactSuccess, (state, { updatedData }) => {
    return {
      ...
      Contact: state.Contact.map((contacts) =>
        contacts._id === updatedData._id ? updatedData : contacts,
      ),
      error: null,
    };
  }),
  ...
  on(updateCompanySuccess, (state, { updatedData }) => {
    return {
      ...state,
      Company: state.Company.map((Companys) =>
        Companys._id === updatedData._id ? updatedData : Companys,
      ),
      error: null,
    };
  }),
  ...
  on(updateLeadSuccess, (state, { updatedData }) => {
    return {
      ...state,
      Lead: state.Lead.map((Leads) => (Leads._id === updatedData._id ? updatedData : Leads)),
      error: null,
    };
  }),
