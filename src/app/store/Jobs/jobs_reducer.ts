...
export const ApplicationReducer = createReducer(
  ...
  on(updateApplicationSuccess, (state, { updatedData }) => {
    return {
      ...state,
      Application: state.Application.map((Applications) =>
        Applications.id === updatedData.id ? updatedData : Applications,
      ),
      error: null,
    };
  }),
  ...
  on(deleteApplicationSuccess, (state, { id }) => {
    const updatedApplication = state.Application.filter(
      (Application) => !id.includes(Application.id),
    );
