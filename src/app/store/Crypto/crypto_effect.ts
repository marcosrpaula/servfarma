...
import {
  fetchCryptoOrderData,
  fetchCryptoOrderFailure,
  fetchCryptoOrderSuccess,
  fetchCryptoTransactionData,
  fetchCryptoTransactionFailure,
  fetchCryptoTransactionSuccess,
} from './crypto_action';
    ...
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.restApiService.getCryptoOrderData().pipe(
          ...
          catchError((error) => of(fetchCryptoOrderFailure({ error }))),
        ),
      ),
    ),
  );
    ...
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.restApiService.getTransactionData().pipe(
          ...
          catchError((error) => of(fetchCryptoTransactionFailure({ error }))),
        ),
      ),
    ),
  );
