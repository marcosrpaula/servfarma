...
export class DryPackageUpsertComponent implements OnInit {
  ...
  title = computed(() => {
    if (this.isReadOnly()) return 'Visualizar Pacote Seco';
    return this.id() ? 'Editar Pacote Seco' : 'Adicionar Pacote Seco';
  });
  ...
  save() {
    ...
    const value = this.form.value as DryPackageInput;
  ...
  private toPackageViewModel(data: PackageViewModel | SimpleItemViewModel): PackageViewModel {
    ...
    return {
      ...
      depth: pkg.depth ?? 0,
      barcode: pkg.barcode ?? '',
      coolingDurationHours: pkg.coolingDurationHours,
