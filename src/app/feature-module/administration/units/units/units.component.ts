...
export class UnitsComponent implements OnInit {
  ...
  constructor(
    private api: UnitsApiService,
    private pagination: PaginationService,
    private unitsState: UnitsStateService,
  ) {
  ...
  ngOnInit(): void {
    ...
    this.allowPagerUpdates = true;
    this.pagination.calculatePageSize.next({ totalData: this.totalItems, pageSize: this.pageSize, tableData: [], serialNumberArray: [] });
    this.loadPage();
  }
  ...
  changePageSize(size: number) {
    ...
    this.backendPage = 1;
    this.pagination.tablePageSize.next({ skip: (this.backendPage - 1) * this.pageSize, limit: this.backendPage * this.pageSize, pageSize: this.pageSize });
  }
  ...
  private loadPage() {
    ...
    this.api.list(params).subscribe({
      next: (res) => {
        ...
        this.unitsState.setMany(this.tableData);
