...
import { ListUnitsParams, UnitSortableField, UnitViewModel } from '../../../../shared/models/units';
import {
  UnitListFiltersState,
  UnitSortLabel,
  UnitsListState,
  UnitsStateService,
} from '../services/units-state.service';
import { UnitsApiService } from '../services/units.api.service';

const SORT_FIELD_MAP: Record<UnitSortLabel, UnitSortableField> = {
...
export class UnitsComponent implements OnInit {
  private readonly api = inject(UnitsApiService);
  private readonly pagination = inject(PaginationService);
  private readonly unitsState = inject(UnitsStateService);
  private readonly fb = inject(NonNullableFormBuilder);
  private readonly destroyRef = inject(DestroyRef);

  readonly breadCrumbItems = [{ label: 'Catálogos' }, { label: 'Unidades', active: true }];
  ...
  readonly tableData = signal<UnitViewModel[]>([]);
  ...
  ascending = false;
  orderLabel: UnitSortLabel = 'CreatedDate';
  ...
  ngOnInit(): void {
    ...
    const savedState = this.unitsState.getListState();
    if (savedState) {
  ...
  toggleSort(field: UnitSortLabel): void {
  ...
  private restoreFromState(state: UnitsListState): void {
    ...
    this.api
      ...
      .subscribe({
        next: (res) => {
          ...
          this.unitsState.setMany(items);
          this.unitsState.setListState({
            items,
  ...
  private getFilters(): UnitListFiltersState {
